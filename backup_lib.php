 ...
 * @return array ocorrências: cada uma ['chave','titulo','loja','detalhe']
 *
 * Nível de MÁQUINA (servidor), não por política — decisão consciente:
 * um servidor de backup roda várias políticas com frequências bem diferentes
 * (ex.: uma 2x/dia, outra 1x/semana), então não existe um limiar de horas
 * único que faça sentido política a política — a semanal ia "silenciar"
 * todo santo dia entre uma execução e outra. "Sem contato" aqui significa
 * "o servidor inteiro parou de falar" (nenhuma política, nem sucesso nem
 * erro): o limiar (silencio_horas, por máquina) deve ser calibrado pela
 * política MAIS FREQUENTE daquele servidor — as outras continuam vivas por
 * tabela enquanto pelo menos uma reportar dentro do prazo.
 */
function alerta_check_backup_silencio(PDO $pdo, array $p): array
{
    ...
    foreach ($st->fetchAll(PDO::FETCH_ASSOC) as $m) {
        if ($m['ultimo_contato'] === null) {
            ...
            $detalhe = 'sem contato há ' . $horas . 'h (última política a reportar: ' . ($m['ultima_politica'] ?: '?') . ')';
        }
        $out[] = [
