...
> **Progresso (branch `feat/monitor-rede-etapa1`):** ✅ passo 1 `monitor_lib.php` + testes (7f58027) · ✅ passo 2 sincronização + semente do Dude em produção — 144 do inventário, 9 manuais, 65 monitorados, os 40 do Dude cobertos (fdd62f3) · ✅ passo 3 tela `monitor_dispositivos.php` + link (2343bb7) · ✅ passo 4 chave Monitorar no modal "Editar máquina" do inventário (5a560c8) · ⏳ passo 5 validação do usuário na tela + merge. Balanças: chave pela tela do Monitor (tela de balanças fica pra etapa 4, junto com o status por categoria).
