...
| 8 | Duração livre de eventos (15min → 8h) + visual correto na grade | `agenda/index.php` | ✅ CONGELADO |
| 9 | Repositório Git criado e publicado no GitHub | `.gitignore`, `agenda/config.example.php` | ✅ CONGELADO |
...
## Bug 8 — Duração livre de eventos + visual correto na grade

**Problema 1 — Duração fixada em 30 min:**
O modal não permitia alterar a duração. `ajustarDuracaoPorTipo()` e `preencherModal()`
recalculavam o `ev-end` com duração padrão, sobrescrevendo qualquer valor personalizado.

**Solução:**
- Novo campo `ev-duracao` (select: 15min, 30min, 1h, 1h30, 2h, 3h, 4h, 8h, Personalizado)
- Funções `aoMudarInicio()`, `aoMudarDuracao()`, `aoMudarFim()` sincronizam os campos
- `ajustarDuracaoPorTipo()` atualizado para definir duração padrão por tipo
- `salvarEvento()` chama `aoMudarInicio()` antes de ler `ev-end` para garantir valor atual
- `preencherModal()` não mais chama `ajustarDuracaoPorTipo()` no final (evita reset)
- Padrões: Reunião=1h, Chamado/Requisição/Evento=30min

**Problema 2 — Evento salvo com 3h aparecia como 30min na grade visual:**
Investigação via `eventDidMount` debug revelou:
- `harness computedHeight: 150px` ✅ (FullCalendar calculava 3h correto)
- `el.offsetHeight: 25px` ❌ (card não preenchia o harness)

**Causa raiz:** CSS `position: relative !important` (linha 282) sobrescrevia o
`position: absolute` que o FullCalendar aplica em `.fc-timegrid-event` para
esticar o card até preencher o harness. Com `relative`, o card colapsava para
o tamanho do conteúdo texto (~25px).

**Fix:** regra `position: relative !important` removida. Substituída por
`overflow: visible !important` (sem impacto no layout, mantém badge visível).
O `.ev-check-badge` continua funcionando pois o elemento pai `position: absolute`
já serve como containing block.

**Também:** formato ISO 8601 com `T` adicionado ao passar datas ao FullCalendar
(`replace(' ', 'T')` em `carregarEventos()`).

## Nota — Repositório Git

Repositório criado e publicado:
- URL: https://github.com/marcosbarrosdemiranda/portal-glpi
- Arquivos sensíveis protegidos via `.gitignore` (credentials, backups, debug files)
- Template `agenda/config.example.php` incluído para novos deploys
...
| Arquivo | Alteração | Status |
|---------|-----------|--------|
| `agenda/index.php` | Todos os bugs 1–8 | ✅ CONGELADO |
| `glpi_doc_proxy.php` | Método filesystem + sessão web melhorado | ✅ CONGELADO |
| `agenda/config.php` | `GLPI_ABSPATH` adicionado | ✅ CONGELADO |
| `agenda/config.example.php` | Template criado para novos deploys | ✅ CONGELADO |
| `debug_proxy.php` | Diagnóstico expandido | ✅ CONGELADO |
| `.gitignore` | Criado — protege credenciais e backups | ✅ CONGELADO |
