  ...
  calendar = new FullCalendar.Calendar(document.getElementById('calendar'), {
    ...
    dateClick(info) {
      if (dataNoPassado(info.date)) {
        toast('⚠️ Não é possível agendar em datas passadas.', 'danger');
        return;
      }
...
function salvarEventoObj(dados, cb) {
  ...
  .then(() => {
    if (dados.ticket_id) {
      ...
      // ⚠️ ORDEM DE EXECUÇÃO PROTEGIDA — NÃO ALTERAR SEM PERMISSÃO DO RESPONSÁVEL ⚠️
    ...
    .then(res => {
      if (res.ok) {
        ...
        if (ticketId) {
          ...
          .then(r => r.json())
          .then(r => {
            if (r.ok) toast(`↩️ Chamado #${ticketId} voltou para Novo e sem técnico.`);
            carregarTickets(); // sidebar atualizada APÓS o GLPI ter status=1
          })
...
function syncRotinas(manual = false) {
  ...
  fetch('sync_rotinas_ajax.php')
    ...
    .catch(() => { if (manual) toast('⚠️ Erro ao sincronizar rotinas.'); })
    .finally(() => {
  ...
  fetch('google_calendar.php?action=get')
    ...
    .then(d => {
      ...
      document.getElementById('gcal-status').innerHTML = d.url
        ? '<span class="badge bg-success">✓ Google Calendar conectado</span>'
...
function salvarGcal() {
  ...
  .then(d => {
    if (d.ok) {
      document.getElementById('gcal-status').innerHTML = '<span class="badge bg-success">✓ ' + d.msg + '</span>';
      modalGcal.hide();
      carregarEventosGcal();
    } else {
